...
class Page
{
	...
	private $uri;
	private $pageNum; //页数
	private $config=array('header' =>"个人记录", "prev"=>"上一页", "next"=>"下一页", "first"=>"首 页", "last"=>"尾 页");
	private $listNum=8;
	/*
	 * 构造方法 
	 */
	...
	private function pageList(){
		...
		for ($i=1; $i <= $inum; $i++) {//点击页码跳转
			$page=$this->page+$i; 
			if ($page<=$this->pageNum) {
				// $linkPage.="&nbsp;<a href='{$this->uri}&page={$page}'>{$page}</a>&nbsp";
				$linkPage.="&nbsp;<a href='javascript:showpage(\"{$this->uri}&page={$page}\")'>{$page}</a>&nbsp";
			} else {
				break;
			}
